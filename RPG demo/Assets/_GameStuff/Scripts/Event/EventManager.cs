...
{ 
    public class EventManager : MonoBehaviour
    {
        ...
        {
            // 作用：阻止数组index超过数组长度
            if (CalendarManager.m_Instance.m_CurrentDay % 7==0 && isWeekEnd==false)
            {
                isWeekEnd = true;
                CalendarManager.m_Instance.TriggerWeekEndDialog();
                return;
            }
        ...
        public void HandleDayEvent(int dayId)
        {
            ...
            //else
            //{
                PanelManager.m_Instance.RefreshPopup(PanelManager.m_Instance.m_PopupPanel, m_EventArray[dayId%7]);
                PanelManager.m_Instance.OpenPanel(PanelManager.m_Instance.m_PopupPanel);
                m_EventArray[dayId%7].HandleEvent();
                ShowCG(m_EventArray[dayId%7].cg);
        ...
        public void HandleFirstDay()
        {
            ...
            Debug.Log("本周第一天：全局第" + GetCurrentWeek()*7 + "天(从0)");    // 0-1, 7-8, 14-15...
            int a = CalendarManager.m_Instance.m_CurrentDay = GetCurrentWeek() * 7;
            int b = CalendarManager.m_Instance.m_CurrentScheduleDay = (GetCurrentWeek()+1) * 7;
            Debug.LogFormat("m_CurrentDay: {0}\nm_CurrentScheduleDay: {0}\n", a, b);
            HandleDayEvent(firstWeekDay);
        ...
        public void NextWeek()
        {
            ...
            PanelManager.m_Instance.m_ResultWindow.SetActive(false);
        }

        private int GetCurrentWeek()
        {
            return CalendarManager.m_Instance.m_WeekNum-1;
        }
        ...
        internal void AddEventToWishlist(GameObject gameObject)
        {
            ...
            //int lastFreeDay = GetLastFreeDayInWeek(w);
            //if (CheckDayScheduled(lastFreeDay))
            //{
            //    return;
            //}
