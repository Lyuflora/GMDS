...
namespace Gmds
{
    public class GameStatus : MonoBehaviour
    {
        ...
        public RatingSlidingNumber[] m_RatingSlider;
        ...
        public void UpdateProjRatings()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                m_RatingSlider[i].SetNumber(m_GameRatings[i]);
                Debug.Log("Rating " + i + " = " + m_GameRatings[i]);
            }
            m_ArtText.text = m_GameRatings[0].ToString();
            m_BusText.text = m_GameRatings[2].ToString();
            m_MechanicText.text = m_GameRatings[1].ToString();
            m_ReputationText.text = m_GameRatings[4].ToString();
            m_TechText.text = m_GameRatings[3].ToString();
        }
